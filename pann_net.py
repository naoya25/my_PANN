...
class PANN(nn.Module):
    """
    Define the generic physics-in-architecture neural network (PANN) structure
    ...
    def forward(self, inputs, x):
        outputs = []
        _x = x[:, 0]  # initialize the state
        for t in range(inputs.shape[1]):
            state_next = self.cell.forward(inputs[:, t, :], _x)  # infer one time stamp
...
class EulerCell_DAB(nn.Module):
    ...
    def __init__(self, dt, Lr, RL, n, **kwargs):
        ...
        self.Lr = nn.Parameter(
            torch.Tensor([Lr])
        )  # define trainable neural parameter Lr
        self.RL = nn.Parameter(
            torch.Tensor([RL])
        )  # define trainable neural parameter RL
        self.n = nn.Parameter(torch.Tensor([n]))  # define trainable neural parameter n

    def forward(self, inputs, states):
        # inputs : represent vp, vs respectively
        # states : represent iL, respectively
        iL_next = (self.Lr / (self.Lr + self.RL * self.dt)) * states[:, 0] + (
            self.dt / (self.Lr + self.RL * self.dt)
        ) * (inputs[:, 0] - self.n * inputs[:, 1])  # physics of iL
        return iL_next[:, None]
...
class EulerCell_Buck(nn.Module):
    ...
    def forward(self, inputs, states):
        ...
        idx = (inputs[:, 0] == 0) & (
            states[:, 0] <= 0
        )  # evaluate discrete conduction mode
        va[idx] = vo[idx]
