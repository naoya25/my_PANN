...
def create_vpvs(D0, D1, D2, Vin, Vref, Tslen, dt, Tsim, Ts, D1_cycle=0.5, D2_cycle=0.5):
    """
...
def transform(input_, pred, Vin, Tslen, convert_to_mean=False):
    """
    ...
    if isinstance(input_, torch.Tensor):
        ...
        input_o = torch.zeros(input_.shape[0], Tslen, input_.shape[-1]).to(
            input_.device
        )
    elif isinstance(input_, np.ndarray):
        pred_o = np.zeros((input_.shape[0], Tslen, pred.shape[-1]), np.float32)
        input_o = np.zeros((input_.shape[0], Tslen, input_.shape[-1]), np.float32)
    ...
    for i in range(input_.shape[0]):
        ...
        if convert_to_mean:
            pred_o[i, :, 0] -= pred_o[
                i, :, 0
            ].mean()  # eleminate DC bias for steady-state waveforms
    return pred_o, input_o


def get_inputs(D0, D1, D2, D1_cycle, D2_cycle, Vin, Vref, Tslen, dt, Tsim, Ts):
    """
    ...
    for _D0, _D1, _D2, _D1_cycle, _D2_cycle in zip(D0, D1, D2, D1_cycle, D2_cycle):
        ...
        _input = np.concatenate([vp_[None, :, None], vs_[None, :, None]], axis=-1)
        inputs.append(_input)
    return np.concatenate(inputs, axis=0)


def evaluate(inputs, targets, model_pann, Tslen, Vin, convert_to_mean=True):
    """
...
def evaluate_onnx(
    inputs, targets, model_pann_onnx, Tslen, seqlen_onnx, Vin, convert_to_mean=True
):
    """
