...
def train(
    ...
    for epoch in range(epoch):
        ...
        for data in data_loader:
            ...
            state0 = torch.zeros(state.shape).to(
                device
            )  # should be zero to avoid learning the initial state
            ...
            clamper(
                model_pann
            )  # comment out this line if using pure data-driven model for dk
            total_loss += loss_train.item()
        estimated_circuit = list(
            map(lambda x: round(x.item(), 7), model_pann.parameters())
        )
        ...
        if epoch % 1 == 0:
            ...
            *_, test_loss = evaluate(
            ...
            train_test_loss = (
                train_loss * len(state) + test_loss * len(test_states)
            ) / (len(state) + len(test_states))
            if train_test_loss < loss_best_pann:
                # select the best pann checkpoint based on the training and test losses
                loss_best_pann, best_pann_states = (
                    train_test_loss,
                    copy.deepcopy(model_pann.state_dict()),
                )
                print(f"New loss is {loss_best_pann}.")
                print("-" * 81)
            if verbose:
                val_pred, _, val_loss = evaluate(
                    val_inputs[:, 1:],
                    val_states,
                    model_pann,
                    Tslen,
                ...
                val_waveforms.append(
                    val_pred[[0, 3]]
                )  # save the first two waveform predictions
